...
def soundinit():
    ...
    try:
        ...
        for x in range(37):
            ...
            otherchannels = [pygame.mixer.Channel(
                37), pygame.mixer.Channel(38)]
    except pygame.error:
...
for armnum in range(8):
    octopusarmtomultipartpart.append(octopusarmdraworder.index(
        armnum)+1)  # add one because the body is drawn first
...
def save_properties():
    # overestimate
    properties.num_of_generated_graphics = int(
        num_of_generated_graphics_used() * 1.2)
...
def draw_loading_text(string):
    ...
    pygame.draw.rect(screen, BLACK, Rect(xpos-text.get_width(),
                     ypos, text.get_width()*3, text.get_height()*2))
...
def draw_loading_tips():
    if devoptions.args.novideomode:
        return
    text = pygame.transform.scale2x(font.render(
        "tip: use the escape key to pause the game", 0, WHITE).convert())
    xpos = int((width / 2) - (text.get_width() / 2))
    ypos = int((height / 2) - text.get_height() -
               height/10) - text.get_height()*2.5
    pygame.draw.rect(screen, BLACK, Rect(xpos-text.get_width(),
                     ypos, text.get_width()*3, text.get_height()))
    screen.blit(text, [xpos, ypos])
...
def draw_graphic_name(name):
    if not devoptions.args.novideomode:
        ...
        textrect = Rect(xpos-text.get_width(), ypos,
                        text.get_width()*3, text.get_height())
...
def draw_progress_bar():
    ...
    if not estimated == None:
        ...
        progresstext = pygame.transform.scale2x(font.render(
            str(numused) + "/" + str(estimated), 0, WHITE).convert())

        texty = int(height/2 + height/5)

        screen.fill(BLUE, Rect(0, int(height/2),
                    width*percent_complete, height/10))

        screen.fill(BLACK, Rect(0, int(height/2 + height/10), width,
                    int(height/2 + progresstext.get_height()*2)))

        screen.blit(progresstext, [
                    int(width/2 - progresstext.get_width()/2), texty])
        pygame.display.flip()
...
def updaterects():
    ...
    pygame.display.update(dirtyrects + olddirtyrects + [Rect(
        10, font.get_linesize(), font.get_linesize()*5, font.get_linesize()*3)])
