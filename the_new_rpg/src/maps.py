...
househeight = graphics.house["scale-height"]
hw = graphics.honey_right0["scale-width"]
hh = graphics.honey_right0["scale-height"]

# outside 1
outside1 = Map(graphics.scrub1, [houserock,
                                 Rock(graphics.tree3, 1.4*block, 1.9*block, True),
                                 Rock(graphics.meanGreen0, 3.2*block, 0.1*block+graphics.tall_Tree["scale-height"]-graphics.meanGreen0["scale-height"], True),
                                 Rock(graphics.rock, 6.5*block, 7*block, True),
                                 Rock(graphics.rock, 4.5*block, 3.5*block, True),
...
outside1.foreground_terrain = [Rock(graphics.tall_Tree, 2.9*block, 0.1*block, False)]
...
racoonc.area = [0, 7*insideb+graphics.tpanda["scale-height"], insidewidth, 50]#50 because it does not matter how thick it is down
